...
def sellCrop(request):
    ...
    try:
        if request.method=="POST":
            ...
            farmer_name = request.user.username   # 🔥 AUTO SET
...
def buyier(request):
    ...
    if request.method == "POST":
        ...
        if buyer_name and crop_name:
            Buyier.objects.create(
                user=request.user,  # ✅ VERY IMPORTANT FIX
    ...
    if request.method == 'POST':
        ...
        if accept_id:
            buyer = get_object_or_404(
                Buyier,
                id=accept_id,
                farmer_name=request.user.username   # 🔥 SECURITY FIX
            )
            buyer.status = "accepted"
            buyer.save()

            return redirect('/storage?success=1')   # ✅ success message
        ...
        # DELETE ACCEPTED (BY BUYER)
        delete_accepted_id = request.POST.get('delete_accepted_id')
        if delete_accepted_id:
            buyer = get_object_or_404(
                Buyier,
                id=delete_accepted_id,
                user=request.user   # ✅ only his own request
            )
            buyer.delete()
            return redirect('storage')

    # ✅ SELLER VIEW → only pending
    buyers = Buyier.objects.filter(
        farmer_name=request.user.username,
        status="pending"
    )

    # ✅ BUYER VIEW → only his accepted
    accepted_requests = Buyier.objects.filter(
        user=request.user,
        status="accepted"
    )
...
def object_names_stream(request):
    def event_stream():
        ...
        while True:
            ...
            # 🚀 send alert only every 10 seconds
...
def workers(request):
    ...
    if request.GET.get('start') == 'true':
        ...
        if new:
            frs = FaceRecognitionSystem()   # ✅ fresh instance
            frs.capture_faces(new)
            frs.train_model()
    ...
    if request.method == "POST":
        ...
        try:
            response = requests.post(
                ...
                headers={
                    "Authorization": "Bearer add the api key ",  # ✅ FIXED
                    "Content-Type": "application/json"
                },
                json={
                    "model": " add the model name ",  # ✅ NEW MODEL
                    "messages": [
